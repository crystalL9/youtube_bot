...
class DetailCrawler:
    ...
    def extract_video_info_json(self, main_key, sub_key,mode):
        ...
        try:
            ...
            try: 
                ...
                video_info["like"] = int(self.extract_like_count(
            ...
            try:
                ...
                while not comment_element.is_displayed():
    ...
    def detect_views_and_upload_info(self, context):
        ...
        #     r"([\d,]+)\s+views\s+Premiered\s+(\d+)\s+minutes ago",
        #     r"([\d,]+)\s+views\s+Premiered\s+(\d+)\s+hours ago"
        # ]
        ...
        #                     published_time = current_time.replace(minute=current_time.minute - minutes_ago)
        #                 if "hours" in pattern:
        #                     published_time = current_time.replace(hour=current_time.hour - minutes_ago)
